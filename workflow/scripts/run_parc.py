...
#
# Phase: clustering
# Step: perform clustering with the ESCHR algorithm (hyperparameter-randomized ensemble clustering)
# Original publication : 10.1186/s13059-024-03386-5
# 
# Author: Émilie Roy
# Date: Jan 2026
# Version: v.1.0
...
# Outputs : standard output clusters.csv + eschr model
#==================================================================================================================#
...
jac_std_global = config_file["jac_std_global"]
jac_weighted_edges = config_file["jac_weighted_edges"]

if isinstance(jac_std_global, str):
    if jac_std_global != "median":
        sys.exit(f"In {os.path.basename(clustering_config)}: {jac_std_global} is not a valid choice. jac_std_global can be set to 'median' or a value between 0.1 and 1.")
if isinstance(jac_std_global, float):
    if jac_std_global <= 0:
        sys.exit(f"In {os.path.basename(clustering_config)}: {jac_std_global} is not a valid choice. jac_std_global can be set to 'median' or a value between 0.1 and 1.")
check_boolean(jac_weighted_edges, "jac_weighted_edges", clustering_config)
...
print(f"Running PARC with jac_std_global={jac_std_global}, and jac_weighted_edges={jac_weighted_edges}... \n", flush=True)
...
if method_choice.lower() == "direct_markers":
    parc_model = {
        ...
        "jac_weighted_edges" : jac_weighted_edges,
        "jac_std_global" : jac_std_global,
        "f1_mean" : parc1.f1_mean,
        "stats_df" : parc1.stats_df,
...
with open(model_pkl, "wb") as f:
    pickle.dump(parc_model, f)

print_save_msg(filepath=model_pkl, filename="PARC model")
...
small_df["cluster"] = pd.Categorical(parc_labels)
