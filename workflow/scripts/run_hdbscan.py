...
# Original publicatipon : https://doi.org/10.1007/978-3-642-37456-2_14
#
# Author: Émilie Roy
# Date: Jan 2026
# Version: v.1.0
...
from clustering_utils import (
    ...
    print_cluster_size_bef_aft_propagation, save_standard_clustering_output, check_list_strings,
    check_boolean, check_string, fit_transform_dr
    )
...
min_samples = config_file["min_samples"]
min_cluster_size = config_file["min_cluster_size"]

check_pos_int_not_null(sample_size, "sample_size", clustering_config)
check_pos_int_not_null(min_samples, "min_samples", clustering_config)
check_pos_int_not_null(min_cluster_size, "min_cluster_size", clustering_config)
...
# run hdbscan on downsampled dataset
clustering_start = datetime.now()
print("Clustering start:", clustering_start.strftime("%Y-%m-%d %H:%M:%S"), flush=True)
print(f"Running HDBSCAN with min_samples={min_samples}, and min_cluster_size={min_cluster_size}... \n", flush=True)
...
with open(model_pkl, "wb") as f:
    pickle.dump(hdb_model, f)

print_save_msg(filepath=model_pkl, filename="HDBSCAN model")
...
small_df["cluster"] = pd.Categorical(hdb_labels)
...
# Transform and propagate labels by chunk
